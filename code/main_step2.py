...
def train(model_gen, model_dis,params):
    ...
    scheduler = CosineAnnealingLR(opt_gen, T_max=len(train_dl), eta_min=0, last_epoch=-1)
    scheduler_D = CosineAnnealingLR(opt_model_dis, T_max=len(train_dl), eta_min=0, last_epoch=-1)

    
    #%% loss
    loss_func_pix = nn.MSELoss().to(device)
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_dl, unit="batch") as tepoch:
            ...
            for HR in tepoch:
                ...
                HR = augmentation(HR)
                LR = F.interpolate(HR, scale_factor=0.125, mode='bicubic')
                
           
                
                valid = Variable(torch.ones(HR.size(0),1,patch_h,patch_w, requires_grad=False).to(device)-noise_factor*torch.rand(HR.size(0),1,patch_h,patch_w, requires_grad=False).to(device))
                fake = Variable(torch.zeros(HR.size(0),1,patch_h,patch_w, requires_grad=False).to(device)+noise_factor*torch.rand(HR.size(0),1,patch_h,patch_w, requires_grad=False).to(device))
                ...
                pixel_loss = loss_func_pix(SR,HR)
                ...
                if batch_count % 1000 == 0:   
                    ...
                    for ii in range(0,3):
                        ...
                        plt.imshow(to_pil_image(LR_show[ii]),cmap = 'gray')
                        plt.axis('off')
                        if ii<1: plt.title('Input')
                        plt.subplot(3,3,3*ii+2)
                        plt.imshow(to_pil_image(SR_show[ii]),cmap = 'gray')
                        plt.axis('off')        
                        if ii<1: plt.title('x8')
                        plt.subplot(3,3,3*ii+3)
                        plt.imshow(to_pil_image(HR_show[ii]),cmap = 'gray')
                        plt.axis('off')
...
path2weights_gen = os.path.join(path2models, 'weights_gen_last.pt')
